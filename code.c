...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        float *result = (float*)malloc(MATRIX_SIZE * MATRIX_SIZE * sizeof(float));
               for (int i = 0; i < NUM_MAPPERS; i++) {
            MPI_Recv(result + i * num_entries_per_reducer, num_entries_per_reducer, MPI_FLOAT, i + NUM_MAPPERS + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
	...
	printf("Process %d received task map on %s\n", rank, rank <= 3 ? "master" : "slave");
        // Send results to reducers
        for (int i = 0; i < NUM_MAPPERS; i++) {
            MPI_Send(output + i * num_entries_per_reducer, num_entries_per_reducer, MPI_FLOAT_INT, i + NUM_MAPPERS + 1, 0, MPI_COMM_WORLD);
        }
